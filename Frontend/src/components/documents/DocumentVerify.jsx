...
const DocumentVerify = ({ documentId }) => {
  ...
  const verifyDocument = async () => {
    ...
    try {
      ...
      setError(
        error.response?.data?.detail || "Document verification failed."
      );
    } finally {
      setLoading(false);
    }
  ...
  // Backend returns: { integrity: "VERIFIED" | "COMPROMISED", stored_hash: "..." }
  const isVerified = verification?.integrity === "VERIFIED";
  const currentHash = verification?.stored_hash;
      ...
      <div className="border-b border-slate-200 p-5">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100">
            <FiShield className="text-slate-700" size={21} />
          </div>
          <div>
            <h2 className="font-semibold text-slate-900">Document Integrity</h2>
      ...
      <div className="space-y-5 p-5">
        ...
        {!verification && !error && (
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-6 text-center">
            <FiShield className="mx-auto mb-3 text-slate-400" size={34} />
            <p className="font-medium text-slate-700">Document verification</p>
